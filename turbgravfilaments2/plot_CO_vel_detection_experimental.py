...
for fil in rectdata:
    ...
    print specfield[spany//2,spanz//2]
    ax0.plot(vels,specfield[spany//2,spanz//2])
    ax0.set_xlim(-1,1)
    plt.savefig('nonsmoothed.png', dpi=400)
    specfieldsmooth = np.zeros(len(vels))
    thisspecfield = np.zeros(len(vels))
    dvel = vels[1] - vels[0]
    veledges = np.arange(vels[0] - dvel/2, vels[-1] + dvel, dvel)
    for i in xrange(len(vels)):
        peak = vels[i]
        sigma = 0.052
        amp =specfield[spany//2,spanz//2,i] 
        erfdenom = np.sqrt(2*sigma**2)
        cdfs = 0.5 * (1 + special.erf((veledges - peak) / erfdenom)) * amp
        weights = np.roll(cdfs, -1) - cdfs
        specfieldsmooth = specfieldsmooth + weights[:-1]
    print specfieldsmooth
    print vels
    print veledges
    print dvel
    ax0.plot(vels,specfieldsmooth)
    plt.savefig('smoothed.png', dpi=400)
    sys.exit()
   ...
   # velmax = 2
    ...
    plt.savefig('detection'+str(filnum)+'.png', dpi=400)
    plt.clf()
