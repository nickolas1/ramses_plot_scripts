...
for fil in rectdata:
    ...
    if plotSurfaceDensity:
        ...
        expandfac = 8 # resample the surface density map to make off-axis pixels look nice
    ...
    specfile = fileprefix+'posvel_'+str(axis)+'/spectra_C18O_0000.hdf5'
    ...
    expandfac = 8 # resample the surface density map
    ...
    specfielddet = np.zeros([spany, spanz, len(vels)])
    
    # get all of the spectra in this box into an array for easy access
    for i in xrange(zlo, zhi, 1):
        specfile = fileprefix+'posvel_'+str(axis)+'/spectra_C18O_'+str(np.remainder(i, res_spec)).zfill(4)+'.hdf5'
        f = h5py.File(specfile)
        specs = f['spectraC18O']
    ...
    if DetectedOnly:    
        # get 'detected' values
        # first find rms value of this cube
        rms = np.sqrt( np.mean( specfield**2 ) )
        print rms
        # set everything below the rms to zero
        specfield[specfield < rms] = 0
        for i in xrange(zlo, zhi, 1):
            for s in xrange(ylo, yhi, 1):
                extrema = argrelextrema(specfield[s-ylo, i-zlo], np.greater)
               # specfielddet[s-ylo, i-zlo, extrema] = 1
                specfielddet[s-ylo, i-zlo, extrema] = specfield[s-ylo, i-zlo, extrema] 
        specfield = specfielddet
    ...
    if DetectedOnly2:    
        ...
        print 'rms = ',rms
        # set everything below the rms to zero
        specfield[specfield < 2*rms] = 0
        for i in xrange(zlo, zhi, 1):
            for s in xrange(ylo, yhi, 1):
                extrema = argrelextrema(specfield[s-ylo, i-zlo], np.greater)[0]
               # specfielddet[s-ylo, i-zlo, extrema] = 1
                if len(extrema) > 0:
                    for ex in xrange(len(extrema)):
                        startval = specfield[s-ylo, i-zlo, extrema[ex]]
                        cumtot = startval * vels[extrema[ex]]
                        cumtotm = startval
                        start = extrema[ex]
                        accept = True
                        lo = start - 1
                        while specfield[s-ylo, i-zlo, lo] > startval - rms:
                            cumtot += specfield[s-ylo, i-zlo, lo] * vels[lo]
                            cumtotm += specfield[s-ylo, i-zlo, lo]
                            if specfield[s-ylo, i-zlo, lo] > specfield[s-ylo, i-zlo, lo+1]:
                                accept = False
                            lo -= 1
                        hi = start + 1
                        while specfield[s-ylo, i-zlo, hi] > startval - rms:
                            cumtot += specfield[s-ylo, i-zlo, hi] * vels[hi]
                            cumtotm += specfield[s-ylo, i-zlo, hi]
                            if specfield[s-ylo, i-zlo, hi] > specfield[s-ylo, i-zlo, hi-1]:
                                accept = False
                            hi += 1
                        cumtot /= cumtotm
                        idx = (np.abs(vels - cumtot)).argmin()
                        if accept:
                            specfielddet[s-ylo, i-zlo, idx] = 1
        specfield = specfielddet
   ...
   # velmax = 2
    ...
    #transspecsub = np.log10(transspecsub)
    ...
    print transspecsub
    print 5. * np.median(transspecsub_nonzero),np.percentile(transspecsub_nonzero, 95)
    ax0.imshow(transspecsub.T,
       ...
       # vmax = min(1.0, 5. * np.median(transspecsub_nonzero))
        vmax = np.percentile(transspecsub_nonzero, 95)
       # vmin = 0,
       # vmax = 1
       # cmap = 'gray_r',
       # vmin = -1,
    ...
    plt.savefig('filvel'+str(filnum)+'foo.png', dpi=400)
    plt.clf()
